...
def index(request):
    ...
    posts =  posts.order_by("-created_time").all()
...
def following(request):
    posts = Post.objects.filter(
        user__in=request.user.following.all()).annotate(likes=Count('liked_by'))
    posts =  posts.order_by("-created_time").all()
    paginator = Paginator(posts, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    ...
    message =""
    if len(posts) < 1 :
         message = "Please follow some more users :)"
...
def edit_post(request, post_id):
    try:
        ...
        print(new_content)
        post.new_post_content = new_content

        post.save()
        return JsonResponse({'new_post_content': post.new_post_content})
